...
const Effects = {
  ...
  showRuleWarning() {
    ...
    const rules = RuleEngine.currentRules;
    if (!rules) return;

    const warnings = [];

    if (cfg.oneLife) {
      warnings.push({ text: '\u26A0\uFE0F \u8FC7\u52B3\u6A21\u5F0F\uFF1A\u53EA\u67091\u6761\u547D\uFF01\u78B0\u4E00\u4E0B\u5C31\u5C2E', danger: true });
    }
    if (cfg.darkMap) {
      warnings.push({ text: '\U0001F311 \u9ED1\u76D2\u6D4B\u8BD5\uFF1A\u53EA\u80FD\u770B\u5230\u5468\u56F4\u533A\u57DF', danger: false });
    }
    if (cfg.bugSplit) {
      warnings.push({ text: '\U0001F41B Bug\u8513\u5EF6\uFF1ABug\u88AB\u70B8\u540E\u5206\u88C2\u6210\u4E24\u4E2A', danger: false });
    }
    if (cfg.testIsolation) {
      warnings.push({ text: '\U0001F9EA \u6D4B\u8BD5\u9694\u79BB\uFF1A\u70B8\u5F39\u4E0D\u4F24Bug\uFF0C\u53EA\u80FD\u6E05\u5C42\u5C42\u5C71', danger: true });
    }
    if (cfg.timePressure > 0) {
      warnings.push({ text: '\u23F0 \u9650\u65F6\u901A\u5173\uFF1A' + cfg.timePressure + '\u79D2\u5185\u5FC5\u987B\u8FC7\u5173', danger: true });
    }
    if (cfg.chainExplosionBoost) {
      warnings.push({ text: '\U0001F4A5 \u6280\u672F\u503A\u7206\u70B8\uFF1A\u7206\u70B8\u8303\u56F4+1\uFF0C\u5C0F\u5FC3\u70B8\u5230\u81EA\u5DF1', danger: false });
    }
    ...
    const overlay = document.createElement('div');
    overlay.className = 'rule-warning-overlay';

    const box = document.createElement('div');
    box.className = 'rule-warning-box';

    const worldName = rules.worldName || '\u672A\u77E5\u4E16\u754C';
    const title = document.createElement('div');
    title.className = 'rule-warning-title';
    title.textContent = '\u26A0\uFE0F ' + worldName + ' \u2014 \u5371\u9669\u8B66\u544A';
    box.appendChild(title);

    const subtitle = document.createElement('div');
    subtitle.className = 'rule-warning-subtitle';
    subtitle.textContent = '\u672C\u5C40\u4E16\u754C\u89C4\u5219\u5DF2\u751F\u6548\uFF0C\u8BF7\u6CE8\u610F\u4EE5\u4E0B\u673A\u5236';
    box.appendChild(subtitle);

    const list = document.createElement('div');
    list.className = 'rule-warning-list';
    for (const w of warnings) {
      const item = document.createElement('div');
      item.className = 'rule-warning-item' + (w.danger ? ' danger' : '');
    ...
    hint.className = 'rule-warning-hint';
    hint.textContent = '\u70B9\u51FB\u6216\u6309\u4EFB\u610F\u952E\u7EE7\u7EED \u00B7 4\u79D2\u540E\u81EA\u52A8\u6D88\u5931';
